...
describe "Wells API" do
  ...
  describe "#update" do
    ...
    it 'one well' do
      ...
      put "/experiments/#{@experiment.id}/wells/1", params.to_json, {'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json' }
      expect(response).to be_success            # test for the 200 status-code
      json = JSON.parse(response.body)
    ...
    it 'one well that exists' do
      ...
      put "/experiments/#{@experiment.id}/wells/2", params.to_json, {'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json' }
