# polynomial model smoother, borrowed from Friedman's super smoother (Firedman 1984): fit a polynomial model to the data in a sliding window of length approximately `span_frac * 2 * (maximum(x) - minimum(x))` on x axis, set the y value at the middle point of the sliding window as the predicted value by the polynomial model.
...
function pnmsmu(
    ...
    if dlen < 5 || length(Y) != dlen
        error("X and Y must be equal and >= 5 in length.")
    end
