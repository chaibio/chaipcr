...
# write functions to fit MAK2 model here, which will be called in `mod_bl_q` in "amp.jl"


const MAK2_d0_START = 0 # 0 good, 1 bad
const k_START = 10 # used: 10 better, 2 good, 1e-10 bad, 1 bad


function pred_from_d_nm1(::MAK2, d_nm1::Real, k::Real)
...
function pred_from_cycs( # 0.7to1.2e-5 sec for 40 cycles on PC
    ...
    pred_fs = fb + pred_ds[2:end]
    return pred_fs[map(Int, cycs)]
end
...
function fit(
    ...
    @variable(jmp_model, d0 >= 0, start=MAK2_d0_START)
...
#     num_cycs::Integer;
#     d_nm1_str::AbstractString=d_nm1_STR,
#     k_str::AbstractString=k_STR,
#     d0_str::AbstractString=d0_STR
#     )
