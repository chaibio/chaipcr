...
function ensure_ci(
    ...
    if isa(calib_info, Integer)
        ...
        step_ids = sort(unique(MySQL.mysql_execute(db_conn, step_qry)[:step_id]))
        ...
        channels = sort(unique(MySQL.mysql_execute(db_conn, channel_qry)[:channel]))
...
function get_mysql_data_well(
    ...
    found_well_namedtuple = MySQL.mysql_execute(db_conn, qry)
