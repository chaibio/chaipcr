    ...
    if any(map(i -> length(unique(data[i, :])) == 1, 1:num_channels)) # for any channel, all the data points are the same (would result in "AssertionError: !(isempty(grp))" for `kmedoids`)
        ...
        if length(expected_ncg_raw) != 0 # expected genotypes specified for non-control wells
            ...
            while num_genos >= 2 # `p_` = possible
                ...
                for num_expected_ncg in colon(
                                            (num_genos - num_ctrl_genos),
                                            -1,
                                            max(2 - num_ctrl_genos, 0))
        ...
        #
        init_centers, dist_mtx_winit, cluster_result, centers, slhts, slht_mean =
        ...
        if best_num_genos >= max_num_genos - 1 # can call genotypes
            ...
            ## (work liks US medical residency match) find max
            ## for each dimension as reference point, compare centers
            ## to reference point, assign genotype based on min distance
            ## between center/medoid and reference point;
            ## if a center/medoid has min distance (among all centers/medoids)
            ## to multiple reference point, this group is unclassified
            ...
            if num_channels == 2 && all(sum(expected_genos_all[:, new_center_idc], 1) .< 2) 
                ## dual channel, no hetero, only homo1, homo2, ntc
