...
function mc_peak_analysis(
	...
	if size(sn_idc[1])[1]==1
		if size(sn_idc[2])[1]==1
			# rare case that we dont have both left and right nadirs. it shouldnt happen
			# because the results wont be reliable. you should check your input data.
	...
	if isempty(peaks_filtered)
		peaks_filtered=peaks_raw[1]	
	end
    return output_type == McPeakLongOutput ?
        McPeakLongOutput(
