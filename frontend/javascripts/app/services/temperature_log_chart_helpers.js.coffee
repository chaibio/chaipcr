  ...
  (SecondsDisplay) ->
    ...
    @updateData = (temperatureLogsCache, temperatureLogs, resolution, scrollState) ->
      if temperatureLogsCache?.length > 0
        greatest_elapsed_time = temperatureLogsCache[temperatureLogsCache.length-1].temperature_log.elapsed_time
        # greatest_elapsed_time = if greatest_elapsed_time/1000 < 60*5 then 60*5 else greatest_elapsed_time
        left_et_limit = greatest_elapsed_time - (resolution*1000)
