...
window.ChaiBioTech.ngApp.service('SampleTargetDelete', [
  '$window',
  function($window) {
    ...
    angular.element($window).click(function(evt) {
      if(_this.activeDelete && _this.activeDelete.confirmDelete && evt.target.className !== 'delete' && evt.target.className !== 'delete-confirm') {
    ...
    this.deactiveate = function(currentScope) {
      ...
      if(this.activeDelete) {
        if(currentScope.$id !== this.activeDelete.$id) {
          this.activeDelete.confirmDelete = false;
