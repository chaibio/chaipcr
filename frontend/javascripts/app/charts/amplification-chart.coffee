class AmplificationChart extends window.ChaiBioCharts.BaseChart
  ...
  zoomed: ->
    ...
    if (@onZoomAndPan and !@editingYAxis)
      @onZoomAndPan(@zoomTransform, @width, @height, @getScaleExtent())
  ...
  mouseMoveCb: ->
      ...
      max_x = ((@getMaxX() - 1) / (@config.axes.x.max - 1)) * @width
  ...
  zoomTo: (zoom_percent) -> # zoom_percent = {0..1}
    ...
    k = ((@getScaleExtent() - 1) * zoom_percent) + 1
    @chartSVG.call(@zooomBehavior.scaleTo, k)
